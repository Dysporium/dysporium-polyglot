...
import type { Translator, TranslationKey, TranslateOptions, Locale, LocaleChangedEvent } from '@dysporium/polyglot-core';
...
export const TranslationProvider: FC<TranslationProviderProps> = ({
  ...
  useEffect(() => {
    const unsubscribe = translator.on('localeChanged', ({ newLocale }: LocaleChangedEvent) => {
...
export const Trans: FC<TransProps> = ({
  ...
  const { t } = useTranslation();
  const translation = t(i18nKey, { values, count, defaultValue });
